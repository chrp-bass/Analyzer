...
import { expireOnce, healthOnce, ingestOnce, matchBatch, qualityOnce } from "@/lib/song-where/jobs.server";
...
export async function POST(request: Request) {
  ...
  if (stage && ["discovery", "ingest", "expire", "match", "quality", "alert"].includes(stage) &&
  ...
  try {
    ...
    if (stage === "quality") return NextResponse.json(await qualityOnce());
