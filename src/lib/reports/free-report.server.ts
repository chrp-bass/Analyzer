  ...
  return analysisToFreeReport(
    {
      ...
      scores: {
        ...
        performance: (row.scores as Record<string, number>).performance ?? 0,
        arousal: (row.scores as Record<string, number>).arousal ?? 0,
        valence: (row.scores as Record<string, number>).valence ?? 0,
        dominance: (row.scores as Record<string, number>).dominance ?? 0,
        epi: (row.scores as Record<string, number>).epi ?? 0,
      },
